...
def alpha_bar(t):
    """Linear noise schedule: alpha_bar(t) = 1 - t, where t in [0, 1].
    t=0 is clean, t=1 is pure noise."""
    return 1.0 - t
...
def diffusion_loss_fn(model, z_0, left, right):
    """Epsilon-prediction diffusion loss with linear noise schedule.

    z_t = sqrt(alpha_bar(t)) * z_0 + sqrt(1 - alpha_bar(t)) * epsilon
    Loss = MSE(epsilon_theta(z_t, t, c), epsilon)
    """
    B = z_0.shape[0]
    t = mx.random.uniform(shape=(B,))
    epsilon = mx.random.normal(z_0.shape)

    ab = alpha_bar(t)
    ab_exp = ab.reshape(B, 1, 1, 1)

    z_t = mx.sqrt(ab_exp) * z_0 + mx.sqrt(1.0 - ab_exp) * epsilon
    eps_pred = model(z_t, t, left, right)
    return mx.mean((eps_pred - epsilon) ** 2)


def main():
    parser = argparse.ArgumentParser(
        description="Train Classifier-Free Diffusion Guidance (stage 2)")
    parser.add_argument("--epochs", type=int, default=200)
    parser.add_argument("--batch-size", type=int, default=64)
    parser.add_argument("--lr", type=float, default=1e-4)
    parser.add_argument("--checkpoint", type=str, default="diffusion_checkpoint.npz")
    ...
    parser.add_argument("--log", type=str, default="diffusion_train_log.jsonl")
    ...
    loss_and_grad = nn.value_and_grad(model, diffusion_loss_fn)
    ...
    print(f"Training CFDG for epochs {start_epoch}-{start_epoch + args.epochs - 1}")
